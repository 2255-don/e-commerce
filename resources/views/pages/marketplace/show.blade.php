        ...
        <div class="col-12 d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex align-items-center gap-3">
                <span class="badge bg-label-primary p-2 rounded">
                    <i class="ti ti-wallet me-1"></i> Solde:
                    {{ number_format(auth()->user()->wallet->balance ?? 0, 0, ',', ' ') }} FCFA
                </span>
                <a href="{{ route('marketplace.index') }}" class="btn btn-label-secondary"><i
                        class="ti ti-arrow-left me-1"></i> Retour</a>
            </div>
        </div>
                    ...
                    <div class="row">
                                ...
                                <div class="d-flex gap-2 overflow-auto">
                                    @foreach ($product->images as $img)
                                        <a href="{{ asset('storage/' . $img->image_path) }}" target="_blank"
                                            class="border rounded p-1 hover-shadow">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex align-items-center mb-4">
                                <h3 class="text-gold mb-0 me-3 fw-bold">{{ number_format($product->price, 0, ',', ' ') }}
                                    FCFA</h3>
                            ...
                            <div class="d-flex align-items-center mb-4 p-3 bg-lighter rounded">
                                <div class="avatar avatar-md me-3">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($product->seller->shop_name ?? 'Inconnu') }}&background=D4AF37&color=fff"
                                        alt="Avatar" class="rounded-circle">
                                </div>
                                <div>
                                    <h6 class="mb-0">Vendu par</h6>
                                    <span
                                        class="text-heading fw-bold">{{ $product->seller->shop_name ?? 'Vendeur Inconnu' }}</span>
                                    @if ($product->seller && $product->seller->address)
                                        <div class="text-muted small"><i class="ti ti-map-pin me-1"></i>
                                            {{ $product->seller->address }}</div>
                                    @endif
                                </div>
                            </div>
                ...
                <div class="card-body p-0">
                            ...
                            @foreach ($similarProducts as $similar)
                                <li class="list-group-item p-3">
                                    <a href="{{ route('marketplace.show', $similar) }}"
                                        ...
                                        <img src="{{ $similar->thumbnail_url }}" class="rounded me-3 border" width="50"
                                            height="50" style="object-fit: cover;">
                                        <div>
                                            <h6 class="mb-0 text-truncate" style="max-width: 150px;">{{ $similar->title }}
                                            </h6>
                                            <small
                                                class="text-gold fw-bold">{{ number_format($similar->price, 0, ',', ' ') }}
                                                FCFA</small>
                                        </div>
                                    </a>
                                </li>
                    ...
                    <div class="p-3 border-top text-center">
                        <a href="{{ route('marketplace.index', ['category' => $product->category_id]) }}"
                            class="btn btn-sm btn-label-dark w-100">
                            Voir plus
                        </a>
                    </div>
                </div>
