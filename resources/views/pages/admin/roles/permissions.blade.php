...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">{{ __('Administration') }} / {{ __('Roles') }} /</span> {{ $role->nom }} - {{ __('Permissions') }}
</h4>
                                    ...
                                    <div class="mb-3 pb-3 border-bottom">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label fw-bold" for="permission-{{ $permission->id }}">
                                                {{ $permission->name }}
                                                <span class="badge ms-2" style="background-color: #B8860B; color: white;">
                                                    {{ $permission->features_count ?? 0 }} {{ __('features') }}
                                                </span>
                                            </label>
                                        </div>
                                        @if($permission->description)
                                            <small class="text-muted d-block mt-1 ms-4">{{ $permission->description }}</small>
                                        @endif
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="text-muted mb-2">
                    ...
                    <strong id="selected-count">{{ $role->permissions->count() }}</strong> {{ __('permissions assigned') }}
                </p>
