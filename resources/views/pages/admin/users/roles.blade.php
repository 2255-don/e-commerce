...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">{{ __('Administration') }} / {{ __('Users') }} /</span> {{ $user->name }} - {{ __('Roles') }}
</h4>
                                    ...
                                    <div class="mb-3 pb-3 border-bottom">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label fw-bold" for="role-{{ $role->id }}">
                                                {{ $role->nom }}
                                                <span class="badge ms-2" style="background-color: #808080; color: white;">
                                                    {{ $role->permissions_count ?? 0 }} {{ __('permissions') }}
                                                </span>
                                            </label>
                                        </div>
                                        @if($role->description)
                                            <small class="text-muted d-block mt-1 ms-4">{{ $role->description }}</small>
                                        @endif
