        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                <select class="form-select" id="type-filter">
                    ...
                    <option value="route" {{ request('type') == 'route' ? 'selected' : '' }}>{{ __('Route') }}</option>
                    <option value="action" {{ request('type') == 'action' ? 'selected' : '' }}>{{ __('Action') }}</option>
                    <option value="ui_element" {{ request('type') == 'ui_element' ? 'selected' : '' }}>{{ __('UI Element') }}</option>
                    <option value="section" {{ request('type') == 'section' ? 'selected' : '' }}>{{ __('Section') }}</option>
                </select>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <i class='bx {{ $feature->module->icon ?? "bx-cube" }} me-2' style="color: {{ $feature->module->color ?? '#B8860B' }}"></i>
                            ...
                            <td>
                                ...
                                @if($feature->description)
                                    <br><small class="text-muted">{{ Str::limit($feature->description, 50) }}</small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="badge bg-label-{{ $color }}">{{ __(ucfirst(str_replace('_', ' ', $feature->type))) }}</span>
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <x-feature-link feature="admin.features.edit" route="{{ route('admin.features.edit', $feature->id) }}" class="dropdown-item">
                                            <i class="bx bx-edit me-1"></i> {{ __('Edit') }}
                                        </x-feature-link>
                                        <form action="{{ route('admin.features.destroy', $feature->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <x-feature-button feature="admin.features.delete" type="button" class="dropdown-item text-danger delete-btn">
