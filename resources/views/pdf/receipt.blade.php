...
<head>
    ...
    <style>
        ...
        .header { text-align: center; margin-bottom: 30px; position: relative; border-bottom: 2px solid #eee; padding-bottom: 20px; }
        .logo { font-size: 24px; font-weight: bold; color: #5F61E6; text-transform: uppercase; margin-bottom: 10px; }
        ...
        .items th { background: #f8f9fa; text-align: left; padding: 10px; border: 1px solid #ddd; }
        .items td { padding: 10px; border: 1px solid #eee; }
        
        .total-section { text-align: right; margin-top: 20px; }
        .total-row { font-size: 1.2em; font-weight: bold; color: #5F61E6; }
        
        .footer { text-align: center; margin-top: 50px; font-size: 12px; color: #777; border-top: 1px dashed #ddd; padding-top: 15px; }
...
<body>
    ...
    <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-radius: 5px;">
        ...
        {{ $order->items->first()->product->seller->sellerProfile->shop_name ?? 'Boutique Certifiée' }}
        @if($order->items->first()->product->seller->sellerProfile->phone)
         <br>Tél: {{ $order->items->first()->product->seller->sellerProfile->phone }}
        @endif
    </div>
    ...
    <div class="footer">
        ...
        <p>Pour toute question concernant cette commande, veuillez contacter le vendeur ou le support {{ config('app.name') }}.</p>
    </div>
</body>
</html>
